...
test('VERSION file contains the 2.0.0-dev.0 pre-release value', () => {
  const version = fs.readFileSync(path.join(ROOT, 'VERSION'), 'utf8').trim();
  assert.equal(version, '2.0.0-dev.0');
  assert.match(version, /^2\.0\.0-dev\.0$/);
});
...
test('readVersion() returns the pre-release value', () => {
  assert.equal(readVersion(), '2.0.0-dev.0');
});

test('getVersionInfo() reports version, LCIM git commit, and schema version', () => {
  const info = getVersionInfo();
  assert.equal(info.version, '2.0.0-dev.0');
